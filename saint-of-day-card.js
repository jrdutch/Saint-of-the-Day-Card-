...
function parseRSS(xml) {
  ...
  if (perr) {
    console.info('Saint card: XML parser error: ' + perr.textContent.replace(/\s+/g, ' ').slice(0, 200));
    return null;
  }
  const items = Array.from(doc.getElementsByTagName('item'));
  if (!items.length) { console.info('Saint card: feed contained no <item> elements'); return null; }

  const item = pickFreshest(items, it => parseFeedDate(it.getElementsByTagName('pubDate')[0]?.textContent));
  if (!item) return null;
  ...
  const title = g('title');
  if (!title) { console.info('Saint card: feed item has no title'); return null; }
  ...
  let bio = tmp.textContent.replace(/\s+/g, ' ').trim();
  if (bio.length > 480) {
...
const FEEDS = [
  'https://ucatholic.com/category/saint-of-the-day/feed/',
...
function pickFreshest(items, pubOf) {
  ...
  if (!best || now - bestT > 48 * 3600 * 1000) {
    console.info(`Saint card: feed has ${items.length} item(s); newest pubDate = ` +
...
function parseRSS2JSON(data) {
  if (!data || data.status !== 'ok' || !Array.isArray(data.items) || !data.items.length) {
    console.info('Saint card: rss2json response not usable (status/items)');
    return null;
  }
  const item = pickFreshest(data.items, it => parseFeedDate(it.pubDate));
  if (!item) return null;
...
async function fetchSaint() {
  ...
  for (const a of FEED_ATTEMPTS) {
    ...
    try {
      ...
      console.info(`Saint card: ${host} → 200, parsed=${!!saint}` +
                   (saint ? '' : `, body starts: ${bodyPreview}`));
      if (saint) return saint;
    } catch (e) {
      console.info(`Saint card: ${host} → ${e.name}: ${e.message}`);
    }
  }
  console.info('Saint card: all feed attempts failed, using cached/embedded data.');
  return null;
}
